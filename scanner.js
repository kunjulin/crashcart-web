...
(function (global) {
  ...
  //
  // 為什麼還留著 qr_code：護理師很容易對錯目標，掃到急救車上那張 QR code。
  // 認得出來才能跟他說「這是急救車的 QR code，不是員工證」；
  // 不認就只是一直掃不到，他不知道自己哪裡錯了。
  //
  // 多認一種 QR 的代價很小。實測單張解碼時間（桌機，1108×365 的裁切）：
  //   五種 14.1ms／兩種 8.8ms／只留 code_39 6.0ms
  // 偵測本來就每 150ms 才跑一次，那 2.8ms 換一句看得懂的錯誤訊息很划算。
  //
  // 別的欄位要掃別種條碼，呼叫 open() 時傳 formats 就好，不用改這裡。
  var FORMATS = ['code_39', 'qr_code'];
